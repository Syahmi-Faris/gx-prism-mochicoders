    ...
    <div className="px-5 pt-6 space-y-5">
      <header>
        <p className="text-xs text-muted-foreground flex items-center gap-1">
          <ShieldAlert size={12} className="text-primary" /> Real-time BNPL intervention
        </p>
        <h1 className="text-2xl font-semibold">Impulse Intercept</h1>
        <p className="text-sm text-muted-foreground">
          Triggered when GXBank detects a BNPL repayment leaving your account — not when you browse.
        </p>
      </header>
      ...
      <Card className="p-4 border-primary/30 bg-primary/5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <Badge className="bg-amber/15 text-amber-foreground border border-amber/30 text-[10px]">
                BNPL Repayment — not a new purchase
              </Badge>
            </div>
            ...
            <p className="text-[12px] text-muted-foreground mt-0.5">
              Identified as:{" "}
              <span className="font-medium text-foreground">BNPL instalment (SPayLater)</span> — due
              today
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl bg-card border border-border p-3 space-y-2.5">
          ...
          {DETECTION_STEPS.map((s, i) => (
            <div key={i} className="flex items-start gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`text-[12px] ${s.active ? "font-semibold text-foreground" : "text-foreground"}`}
                >
                  {s.label}
                </p>
        ...
        <Button
          ...
          className="mt-3 w-full gradient-prism text-primary-foreground border-0 rounded-full"
        >
      ...
      <section>
        ...
        <div className="space-y-2">
          {[
            {
              icon: ShoppingBag,
              label: "Shopee SPayLater · RM 89 due today",
              time: "Auto-synced from GXBank",
              tone: "primary",
              brand: true,
            },
            {
              icon: AlertTriangle,
              label: "Food delivery spike",
              time: "Today 7:12 PM",
              tone: "amber",
            },
            ...
            <Card key={i} className="p-3 bg-card border-border flex items-center gap-3">
              {r.brand ? (
                ...
                <r.icon
                  className={r.tone === "amber" ? "text-amber" : "text-muted-foreground"}
                  size={16}
                />
              )}
              <div className="flex-1 min-w-0">
              ...
              <Button
                onClick={() => setOpen(true)}
                className="h-7 px-3 text-[11px] rounded-full bg-accent text-foreground border-0 hover:bg-accent/80"
              >
                Review
              </Button>
            </Card>
      ...
      <AnimatePresence>
        {open && <BnplInterceptModal onClose={() => setOpen(false)} />}
      </AnimatePresence>
    </div>
  );
}
...
function BnplInterceptModal({ onClose }: { onClose: () => void }) {
  const [stage, setStage] = useState<"review" | "saved" | "paid" | "delayed">("review");
  const redirect = useApp((s) => s.redirectToRecovery);
  const addCaptured = useApp((s) => s.addCaptured);
  const toggleIntercept = useApp((s) => s.toggleIntercept);
  ...
  const handleDelay = () => {
    toggleIntercept("bnpl");
    setStage("delayed");
    toast.success("Decision delayed for 24 hours");
  };
  ...
  return (
    ...
    >
      ...
      >
        {stage === "review" && (
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <Badge className="bg-amber/15 text-amber border border-amber/30 text-[10px]">
                Today
              </Badge>
            </div>
        ...
        {stage === "saved" && (
          <div className="text-center py-2">
            ...
            <div className="mt-4 flex items-center justify-center gap-2 text-[11px]">
              <span className="px-2 py-1 rounded-full bg-[#EE4D2D]/10 text-[#EE4D2D]">
                Shopee RM 89
              </span>
              <motion.div
            ...
            <Button
              onClick={onClose}
              className="mt-5 w-full gradient-prism text-primary-foreground border-0 rounded-full"
            >
              Done
            </Button>
          </div>
        ...
        {stage === "paid" && (
          <div className="text-center py-2">
            ...
            <p className="text-[12px] text-muted-foreground mt-1">
              Logged. We'll learn from this decision.
            </p>
            <Button
              onClick={onClose}
              className="mt-4 w-full bg-accent text-foreground border-0 rounded-full hover:bg-accent/80"
            >
              Close
            </Button>
          </div>
        ...
        {stage === "delayed" && (
          <div className="text-center py-2">
            <div className="mx-auto h-14 w-14 rounded-full bg-amber/15 grid place-items-center mb-3">
              <Clock className="text-amber" size={26} />
            </div>
            <p className="font-semibold">Decision delayed for 24 hours</p>
            <p className="text-[12px] text-muted-foreground mt-1">
              GX Prism will remind you before any BNPL penalty applies.
            </p>
            <Button
              onClick={onClose}
              className="mt-4 w-full gradient-prism text-primary-foreground border-0 rounded-full"
            >
              Done
            </Button>
          </div>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between text-[12px]">
      ...
      <span
        className={`${strong ? "font-semibold text-base" : ""} ${tone === "amber" ? "text-amber" : "text-foreground"}`}
      >
        {value}
      </span>
    </div>
